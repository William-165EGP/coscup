...
<summary>Source code of case 3-1</summary>
...
## Disadvantages:
### 1. Ignores Core/NUMA Locality:
Although qspinlock strikes the balance between fairness and cache-coherence traffic in the slow-path, it does not explicitly consider core or NUMA locality.

In the queued slow-path, qspinlock mostly follows a first-come, first-served handoff order.
That is, the successor is usually the next waiter in the MCS queue.

This may cause performance issues on NUMA systems.
For example, consider the following handoff sequence:
...
This diagram shows the order of lock handover between waiters.
This handover alternates between NUMA node 0 and NUMA node 1, so it ignores locality.

As a result, the lock cacheline, and possibly protected data, may repeatedly propagate between NUMA nodes. 
This can introduce extra cache-coherence overhead, remote cache access latency, and more cache misses.

Several NUMA-aware locks try to address this problem.
We will discuss two NUMA-aware locks, `CNA` and `RON`, later.

## Paravirtualisation Support
Queue-based locks, such as ticket-locks and MCS locks, need paravirtualisation support in virtualised environments.
If paravirtualisation is not available, they should at least fall back to a test-and-set style lock.

The main problem is vCPU preemption.

Imagine that the successor in the lock queue is running on a vCPU that gets scheduled out by the hypervisor.
Other vCPUs behind this successor cannot bypass it, because queue-based locks usually preserve the handover order.
As a result, those vCPUs may waste CPU time spinning while waiting for a successor that is not currently running.

A test-and-set lock does not suffer from this problem in the same way.
Even if one vCPU is scheduled out, other running vCPUs can still contend for the lock directly.
That is less fair, but it avoids being blocked by a preempted queue successor.

Therefore, qspinlock needs either PV support to cooperate with the host hypervisor, or a fallback to a test-and-set style lock when running in a virtualised environment without PV support.

The related patch can be found [here](https://lore.kernel.org/all/1397747051-15401-18-git-send-email-Waiman.Long@hp.com/).
